...
describe('LockService', () => {
  ...
  it('should make a call to get locks', () => {
    ...
    service.aquireLock({resource_id: ['1', '2'], type: 'number', reset_enabled: true}).subscribe((response) => {
      expect(response).toBe(result);
    });

    const req = httpMock.expectOne(`${config.endpoint}/lock/aquire_lock`);
  ...
  it('should make a call to remove locks', () => {
    ...
    const req = httpMock.expectOne(`${config.endpoint}/lock/release_lock`);
  ...
  it('should make a call to get lock info', () => {
    ...
    const req = httpMock.expectOne(request => request.method === 'GET' && request.url === `${config.endpoint}/lock/lock_info`);
